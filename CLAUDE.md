...
- Static-analysis gates: `go vet ./...`, a `gofmt -l` check, and `staticcheck ./...` (default checks — so error strings are lowercase and unpunctuated per ST1005, except acronym-initial ones like "AI ..."). CI (`.github/workflows/ci.yml`) runs these, then unit (`go test ./internal/...`) and e2e separately, merging coverage; e2e needs `glow` on PATH.
